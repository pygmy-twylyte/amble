...
use amble_script::{
    ActionAst, ConditionAst, compile_goals_to_toml, compile_rooms_to_toml, compile_triggers_to_toml, parse_goals,
    parse_program_full,
};
...
fn run_compile(args: &[String]) {
    ...
    let goals = parse_goals(&src).unwrap_or_else(|e| {
        eprintln!("goal parse error: {}", e);
        process::exit(1);
    });
    for t in &asts {
    ...
    if !rooms.is_empty() {
        match compile_rooms_to_toml(&rooms) {
            Ok(toml) => {
                ...
                } else if asts.is_empty() && out_path.is_none() {
